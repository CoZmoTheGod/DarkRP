...
// Imported from scene staging; this component still relies on low-level legacy rendering APIs.
#pragma warning disable CS0612, CS0618, CS0672

[Alias( "shatter_glass" )]
public sealed class ShatterGlassEntity : Component, Component.ExecuteInEditor, Component.IDamageable
{
	private static readonly List<Vector2> DefaultPoints =
	[
		new( -50, -50 ),
		new( 50, -50 ),
		new( 50, 50 ),
	...
	private sealed class Shard : IValid
	{
		...
		public bool IsValid => PhysicsBody.IsValid() && (Application.IsDedicatedServer || SceneObject.IsValid());
	...
	private System.Random ShatterRandom;
	...
	public float RebuildDelay { get; set; } = 300.0f;
	...
	{
		foreach ( var point in GetValidPoints() )
		{
	...
	{
		var basePoints = GetValidPoints();
		var points = IsPathClockwise( basePoints ) ? basePoints.Reverse<Vector2>().ToList() : basePoints.ToList();
	...
	protected override void OnFixedUpdate()
	{
		...
		if ( !AutoRebuild || !IsBroken || RebuildDelay <= 0.0f )
			return;

		if ( TimeSinceBroken < RebuildDelay )
			return;
	...
	private Shard CreateShard( Transform transform, List<Vector2> points )
	{
		...
		if ( !Application.IsDedicatedServer )
		{
			...
			sceneObject = new SceneObject( Scene.SceneWorld, model, transform );
	...
	public void OnDamage( in DamageInfo damage )
	{
		...
		var worldPosition = damage.Position;
		if ( worldPosition.IsNearlyZero() )
			worldPosition = WorldPosition;

		var impulse = worldPosition - damage.Origin;
		if ( impulse.IsNearlyZero() )
			impulse = WorldTransform.Rotation.Forward;

		impulse = impulse.Normal * damage.Damage * 10.0f;
		ShatterAt( worldPosition, impulse, System.Random.Shared.Next() );
	}

	[Rpc.Broadcast]
	private void ShatterAt( Vector3 worldPosition, Vector3 impulse, int seed )
	{
		var shard = FindShardAt( worldPosition );
		if ( !shard.IsValid() )
			return;

		var localPosition = shard.PhysicsBody.Transform.PointToLocal( worldPosition );
		ShatterRandom = new System.Random( seed );
		var shattered = ShatterLocalSpace( shard, new Vector2( localPosition.x, localPosition.y ), impulse );
		ShatterRandom = null;

		if ( shattered && Networking.IsHost )
		{
			IsBroken = true;
			TimeSinceBroken = 0;
		}
	...
	private int RandomShardInt( int min, int max )
	{
		return (ShatterRandom ?? System.Random.Shared).Next( min, max + 1 );
	}

	private float RandomShardFloat( float min, float max )
	{
		return min + (ShatterRandom ?? System.Random.Shared).NextSingle() * (max - min);
	}

	private Shard FindShardAt( Vector3 worldPosition )
	{
		foreach ( var shard in Shards.Values )
		{
			if ( !shard.IsValid() )
			...
			var localPosition = shard.PhysicsBody.Transform.PointToLocal( worldPosition );
			if ( MathF.Abs( localPosition.z ) > MathF.Max( Thickness, 4.0f ) )
				continue;

			if ( shard.IsPointInside( new Vector2( localPosition.x, localPosition.y ) ) )
				return shard;
		}
	...
	private bool ShatterLocalSpace( Shard shard, Vector2 position, Vector3 impulse )
	{
		if ( !shard.IsValid() )
			return false;
		...
		foreach ( var newShard in shards )
		{
			...
			if ( isLoose || !IsPathOnEdge( newShard.Points, GetValidPoints(), 0.1f ) )
			{
	...
	private List<Shard> GenerateShatterShards( Vector2 stressPosition, IList<Vector2> points, Transform transform )
	{
		...
		int numSpokes = Math.Max( 3, RandomShardInt( shatterType.SpokesMin, shatterType.SpokesMax ) );
		...
		{
			float spokeRadians = (i * segmentRange) + (RandomShardFloat( limitedRangeDeviation * -0.5f, limitedRangeDeviation * 0.5f ) * 0.9f);
		...
		for ( int spokeIndex = 0; spokeIndex < spokes.Count; spokeIndex++ )
		{
			...
			var tipPoint1 = Vector2.Lerp( subShard[0], subShard[1], RandomShardFloat( shatterType.TipScaleMin, shatterType.TipScaleMax ) );
			var tipPoint2 = Vector2.Lerp( subShard[0], subShard[^1], RandomShardFloat( shatterType.TipScaleMin, shatterType.TipScaleMax ) );

			centerHoleVertices.Add( Vector2.Lerp( tipPoint1, tipPoint2, 0.5f ) );

			if ( shatterType.TipSpawnChance > 0 && RandomShardFloat( 0, shatterType.TipSpawnChance ) < 1.0f )
			{
			...
			if ( shatterType.SecondTipSpawnChance > 0 && RandomShardFloat( 0, shatterType.SecondTipSpawnChance ) < 1.0f )
			{
				var secondTipPoint1 = Vector2.Lerp( tipPoint1, subShard[1], RandomShardFloat( 0.2f, 0.5f ) );
				var secondTopPoint2 = Vector2.Lerp( tipPoint2, subShard[^1], RandomShardFloat( 0.2f, 0.5f ) );
			...
			{
				var vecBetweenCorners = Vector2.Lerp( Vector2.Lerp( tipPoint1, tipPoint2, RandomShardFloat( 0.4f, 0.6f ) ), stressPosition, RandomShardFloat( 0.1f, 0.3f ) );
